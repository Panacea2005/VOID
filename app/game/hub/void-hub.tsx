...
const realms = [
  {
    ...
    description:
      "A realm of reflective surfaces and echoing sounds, where players confront distorted versions of their past choices.",
  ...
  },
  {
    ...
    description:
      "The central hub where all realms connect. A vast network of pathways and nodes, representing the interconnectedness of all experiences.",
  ...
  },
  {
    ...
    description:
      "A realm of vast emptiness punctuated by moments of intense beauty. Players navigate through darkness, discovering hidden meanings.",
  ...
  },
  {
    ...
    description:
      "A realm pulsing with energy and life. Players must synchronize with the rhythm of this world to progress, creating harmonies.",
  ...
  },
  {
    ...
    description:
      "A realm of puzzles and cryptic messages. Players decipher ancient codes to unlock the secrets of VOID's creation and purpose.",
  ...
  },
  {
    ...
    description:
      "A realm of falling block puzzles where shapes must be arranged in perfect harmony. Navigate spatial dimensions to clear lines and restore order from chaos.",
  ...
  },
  {
    ...
    description:
      "A mesmerizing realm where players manifest their imagination through pixel art creation. Craft intricate pixel masterpieces that come alive within the digital canvas.",
  ...
  },
  {
    ...
    description:
      "A mysterious realm at the edge of perception. Strange geometries and fractal patterns suggest access to dimensions beyond conventional understanding.",
...
const VoidHub: React.FC<VoidHubProps> = ({
  ...
  const handleCubeCollectionUpdate = useCallback(
    (collection: any[]) => {
      console.log(
        "Received updated cube collection:",
        collection.length,
        "cubes"
      );
      combinedCubeCollectionRef.current = collection;

      // Update the selected cube data when collection changes
      updateSelectedCubeData(selectedCubeId, collection);
    },
    [selectedCubeId]
  );
    ...
    (cubeId: string, cubeCollection: any[]) => {
      ...
      if (selectedCube) {
        console.log(
          ...
          selectedCube.name,
          "Is NFT:",
          selectedCube.isNFT || false
        );
        setSelectedCubeColors(selectedCube.colors);
        setSelectedCubeGlow(selectedCube.glow);
        setSelectedCubeBorder(selectedCube.borderColor);
      } else {
        // Fallback to default cube if not found
        console.log("Cube not found, using default");
        const defaultCube = cubeCollection[0];
        setSelectedCubeColors(defaultCube.colors);
        setSelectedCubeGlow(defaultCube.glow);
        setSelectedCubeBorder(defaultCube.borderColor);
      }
    },
    []
  );
  ...
  useEffect(() => {
    if (combinedCubeCollectionRef.current.length > 0) {
      updateSelectedCubeData(selectedCubeId, combinedCubeCollectionRef.current);
    }
  }, [selectedCubeId, updateSelectedCubeData]);

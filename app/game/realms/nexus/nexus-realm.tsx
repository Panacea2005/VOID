...
const NexusRealm: React.FC<NexusRealmProps> = ({ 
  ...
  const [pulseHistory, setPulseHistory] = useState<{ x: number, y: number, size: number, color: string, opacity: number }[]>([]);
  ...
  const [activeCubeMode, setActiveCubeMode] = useState<"pulse" | "attract" | "repel">("pulse");
  ...
  useEffect(() => {
    if (gameState === "intro") {
      ...
      console.log("Created", newNodes.length, "nodes and", newConnections.length, "connections");
    }
    ...
    if (pulseHistory.length > 0) {
      const timer = setTimeout(() => {
        setPulseHistory(prev => 
          prev.filter(p => p.opacity > 0.05).map(p => ({
            ...p,
            size: p.size * 1.02,
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isDragging && selectedNode !== null) {
        ...
        if (gravitationalPull || currentLevel >= 3) {
          ...
          if (centerDist > 0) { // Avoid division by zero
        ...
        nodes.forEach(otherNode => {
          if (otherNode.id !== selectedNode) {
            ...
            if (distSq > 0 && distSq < 2500) { // Only apply when close
        ...
        if (difficulty === "advanced" && nodeToMove.requiredAlignment) {
          if (nodeToMove.requiredAlignment === 'angular' || nodeToMove.requiredAlignment === 'both') {
            if (nodeToMove.targetAngle !== undefined) {
              ...
              newX = newX * (1-anglePullStrength) + targetX * anglePullStrength;
              newY = newY * (1-anglePullStrength) + targetY * anglePullStrength;
            }
          }
        }
  ...
  }, [isDragging, selectedNode, nodes, connections, gravitationalPull, difficulty]);
  ...
  const checkResonance = () => {
    ...
    setNodes(prev => {
      const updatedNodes = prev.map(node => {
        ...
        const distanceThreshold = node.targetDistance * (difficulty === "advanced" ? 0.05 : 0.1); // Tighter tolerance in advanced mode
        const distanceResonance = Math.abs(distanceFromCenter - node.targetDistance) <= distanceThreshold;
        
        // Check angle if required
        let angleResonance = true;
        if (node.requiredAlignment === 'angular' || node.requiredAlignment === 'both') {
          if (node.targetAngle !== undefined) {
            const angleThreshold = 0.15; // About 8.6 degrees
            angleResonance = Math.abs(currentAngle - node.targetAngle) <= angleThreshold ||
                            Math.abs(currentAngle - node.targetAngle - 2 * Math.PI) <= angleThreshold;
          }
        }
        
        // Determine if node is in full resonance
        const inResonance = node.requiredAlignment === 'both' 
  ...
  useEffect(() => {
    ...
    const energyPercent = totalPossibleEnergy > 0
      ? (currentEnergy / totalPossibleEnergy) * 100
      : 0;
    
    setNetworkEnergy(energyPercent);
    
    // Calculate stability based on how many nodes and connections are active
    const stabilityPercent = nodes.length > 0
      ? (resonantNodeCount / nodes.length) * 100
      : 0;
    ...
    return Array.from({ length: 60 }).map((_, i) => (
      <motion.div
        ...
        style={{
          ...
          boxShadow: `0 0 ${Math.random() * 8 + 2}px #ec4899`
        }}
      />
    ));
  };
    ...
    return pulseHistory.map((pulse, index) => (
      <div
        ...
        style={{
          ...
          background: `radial-gradient(circle, ${pulse.color}${Math.floor(pulse.opacity * 255).toString(16).padStart(2, '0')} 0%, transparent 70%)`,
        ...
        return (
          <div
            ...
            style={{
              ...
              background: selectedNode !== null ? nodes.find(n => n.id === selectedNode)?.color || '#fff' : '#fff',
              opacity: opacity,
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div
          ...
          style={{
            background: `radial-gradient(circle at center, rgba(236, 72, 153, ${0.05 + (networkEnergy/200)}) 0%, transparent 70%)`,
            filter: 'blur(40px)',
          }}
        />
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center w-full max-w-5xl">
        ...
        >
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <p className="text-xl text-purple-300 font-light">Network Resonance</p>
            <div className="h-px w-16 bg-gradient-to-r from-transparent via-pink-500/50 to-transparent"></div>
          </div>
        </motion.div>
        ...
        >
            ...
            {gameState === "playing" && (
              <>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <div className="text-xl text-amber-300">{Math.floor(networkEnergy)}%</div>
                </div>
              </>
            )}
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {gameState === "playing" && (
              ...
              >
                {selectedNode !== null 
                  ? `Moving Node ${selectedNode + 1} - Find its resonant position` 
        ...
        <div className="flex gap-8">
          ...
          >
            ...
            <ul className="text-gray-300 text-sm space-y-2">
              ...
              <li className="flex items-start gap-2">
                <span className="text-pink-400 mt-1">•</span>
                <span>{difficulty === "advanced" 
                  ? "Position matters in both distance and angle" 
                  : "Each node has a unique resonant distance"}</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="text-pink-400 mt-1">•</span>
            ...
            {gameState === "playing" && (
              ...
              <div className="flex justify-between gap-2">
                ...
                <button
                  onClick={() => switchCubeMode(activeCubeMode === "pulse" ? "attract" : activeCubeMode === "attract" ? "repel" : "pulse")}
          ...
          >
            ...
            >
              ...
              {showHints && gameState === "playing" && nodes.map((node) => (
                <div 
                  key={`ring-${node.id}`}
              ...
              {difficulty === "advanced" && showHints && gameState === "playing" && (
                <>
                ...
                return (
                  <motion.div
                    ...
                    style={{
                      ...
                      boxShadow: conn.isActive ? `0 0 8px ${conn.color}` : 'none',
              ...
              <motion.div
                ref={cubeRef}
                className={`absolute left-1/2 top-1/2 transition-all duration-300 cursor-pointer ${!cubeReady && !cubeEngaged ? 'opacity-80' : 'opacity-100'}`}
                style={{
                  transformStyle: "preserve-3d",
                  transform: `translate(-50%, -50%) translateZ(${pulseActive || cubeEngaged ? '50px' : '30px'}) 
              ...
              {nodes.map((node) => {
                ...
                const glowIntensity = node.inResonance ? 2 : node.isActivated ? 1.5 : 1;
                
                // Calculate node size and elevation based on state
                const nodeSize = 40 * (node.isActivated ? 1.2 : 1) * (isSelected ? 1.3 : 1);
                const nodeElevation = node.inResonance ? 35 : node.isActivated ? 25 : isSelected ? 30 : 15;
                
                return (
                  <motion.div
                    key={`node-${node.id}`}
                    className={`absolute rounded-full ${node.isLocked ? 'cursor-not-allowed' : 'cursor-grab'} 
                    ...
                    whileHover={!node.isLocked ? { scale: 1.1, z: nodeElevation + 5 } : {}}
                  >
                    {/* Node ID label */}
                    ...
                    {showHints && gameState === "playing" && node.requiredAlignment === "both" && node.targetAngle !== undefined && (
                      <motion.div
                        className="absolute pointer-events-none"
                        style={{
                          width: "4px",
                          height: "4px",
                          left: `${(gridSize/2) + Math.cos(node.targetAngle) * node.targetDistance}px`,
                          top: `${(gridSize/2) + Math.sin(node.targetAngle) * node.targetDistance}px`,
                          backgroundColor: node.color,
                          transform: "translate(-50%, -50%)",

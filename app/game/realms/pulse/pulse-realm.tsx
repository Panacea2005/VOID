...
const PulseRealm: React.FC<PulseRealmProps> = ({
  ...
  const [lastHitResult, setLastHitResult] = useState<HitResultType | null>(
    null
  );
  ...
  const handleCubeCollectionUpdate = (collection: any[]) => {
    console.log("Pulse Realm received cube collection:", collection.length);
  ...
  const handleGoodHit = (wave: PulseWave) => {
    ...
    setCombo((prev) => {
      ...
      comboMultiplierRef.current =
        1 + Math.min(Math.floor(newCombo / 10) * 0.5, 5);
      return newCombo;
    });
    ...
    setScore((prev) =>
      Math.floor(prev + baseScore * comboMultiplierRef.current)
    );
  ...
  const handlePulseHit = () => {
    ...
    if (wavesInHitZone.length > 0) {
      ...
      const distanceFromPerfect = Math.abs(
        closestWave.radius - PERFECT_HIT_ZONE_SIZE
      );
      const distanceFromGood = Math.abs(
        closestWave.radius - GOOD_HIT_ZONE_SIZE
      );
  ...
  const updateWaves = (currentTime: number) => {
    setPulseWaves((prevWaves) => {
      return prevWaves
        .map((wave) => {
          ...
          const timeToImpact =
            wave.timestamp - (Date.now() - gameStartTimeRef.current);
          ...
          const easedProgress =
            1 - Math.pow(1 - Math.max(0, Math.min(1, progress)), 2);
          const radius = (1 - easedProgress) * WAVE_RADIUS_MAX;

          // Check if wave is in the hit zone
          const inPerfectHitZone =
            Math.abs(radius - PERFECT_HIT_ZONE_SIZE) < 15;
          const inGoodHitZone =
            !inPerfectHitZone && Math.abs(radius - GOOD_HIT_ZONE_SIZE) < 20;
    ...
    const anyWaveInHitZone = pulseWaves.some(
      (wave) => wave.inHitZone && !wave.hit && !wave.missed
    );
  ...
  const createNewBeat = () => {
    ...
    const activeWaveCount = pulseWaves.filter(
      (w) => !w.hit && !w.missed
    ).length;
    ...
    if (activeWaveCount < 1) {
      ...
      if (selectedTrack.difficulty === Difficulty.Easy) {
        ...
        availableWaveTypes =
          Math.random() > 0.8 ? ["fast", "slow"] : ["normal"];
      ...
      const waveType = availableWaveTypes[
        Math.floor(Math.random() * availableWaveTypes.length)
      ] as "normal" | "fast" | "slow";
      const waveSize = availableWaveSizes[
        Math.floor(Math.random() * availableWaveSizes.length)
      ...
      const arrivalTime =
        (baseArrivalTime * arrivalTimeMultiplier) /
        waveSpeedMultiplierRef.current;
  ...
  const gameLoop = (timestamp: number) => {
    ...
    const elapsed =
      timestamp - (lastTimestampRef.current || gameStartTimeRef.current);
    lastTimestampRef.current = timestamp;

    // Calculate current game time
    const currentGameTime = !audioRef.current
      ? 0
    ...
    if (activeWaveCount === 0 && timeSinceLastBeat >= MIN_BEAT_INTERVAL) {
      ...
      activeBeatIndexRef.current =
        (activeBeatIndexRef.current + 1) % beatPatternRef.current.length;
  ...
  const renderPulseWaves = () => {
    return pulseWaves.map((wave) => {
      ...
      return (
        <motion.div
          key={wave.id}
          className={`absolute rounded-full ${wave.hit ? "opacity-60" : ""} ${
            wave.missed ? "opacity-40" : ""
          }`}
          style={{
            width: `${Math.max(wave.radius * 2 * sizeMultiplier, 0)}px`,
            height: `${Math.max(wave.radius * 2 * sizeMultiplier, 0)}px`,
            border: `${borderWidth}px ${waveStyle.borderStyle || "solid"} ${
              wave.hit ? cubeColor : wave.missed ? "#ef4444" : waveColor
            }`,
            boxShadow: `0 0 ${shadowSize}px ${
              wave.hit ? cubeColor : wave.missed ? "#ef4444" : waveColor
            }`,
            opacity: opacity,
            top: "50%",
      ...
      <>
        ...
        <motion.div
          className={`absolute rounded-full border-4 ${
            isOuterRingActive ? "border-green-400" : "border-green-400/70"
          }`}
          animate={{
        ...
        <motion.div
          className={`absolute rounded-full border-3 ${
            isOuterRingActive ? "border-yellow-400" : "border-yellow-400/60"
          }`}
          animate={{
        ...
        <div
          className={`absolute rounded-full border ${
            isOuterRingActive ? "border-white/60" : "border-white/30"
          }`}
          style={{
      ...
      >
            ...
            <div className="relative">
              ...
              <div
                ...
                style={{
                  ...
                  left: "50%",
                  transform: "translate(-50%, -50%)",
                  background: `radial-gradient(circle, ${cubeColor}${
                    isCubeGlowing ? "80" : "30"
                  } 0%, transparent 70%)`,
                    ...
                    <motion.div
                      ...
                      style={{
                        ...
                        transform: `translate(-50%, -50%) rotate(${
                          i * 120
                        }deg) translateX(${CUBE_SIZE * 0.8}px)`,
                      }}
                    />
                  ))}
                </div>
        ...
        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-black/50 backdrop-blur-sm px-6 py-2 rounded-full border border-blue-500/30 z-20">
          <div className="flex items-center">
            <span className="text-blue-400 font-pixel text-sm">
              PRESS{" "}
              <span className="text-white bg-blue-900/50 px-2 py-0.5 rounded mx-1">
                SPACE
              </span>{" "}
              OR{" "}
              <span className="text-white bg-blue-900/50 px-2 py-0.5 rounded mx-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`font-pixel text-sm ${
                selectedTrack.difficulty === Difficulty.Easy
                  ? "text-green-400"
                  : selectedTrack.difficulty === Difficulty.Medium
                  ? "text-yellow-400"
                  : "text-red-400"
              }`}
            >
        ...
        <div className="absolute left-4 top-1/2 transform -translate-y-1/2 flex flex-col gap-4 z-30">
          ...
          <div className="bg-black/60 backdrop-blur-md p-3 rounded-2xl border border-green-500/30 text-center shadow-lg w-36">
            <div className="text-green-300 font-pixel text-xs mb-1">
              ACCURACY
            </div>
            <div
              className={`text-2xl font-bold transition-colors ${
        ...
        <div
          ...
          style={{
            background: `radial-gradient(circle at ${mousePosition.x}% ${mousePosition.y}%, ${selectedCube.colors[0]}15 0%, transparent 70%)`,
            filter: "blur(40px)",
          }}
        />
      </div>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          ...
          style={{
            ...
            transform: "translate(-50%, -50%)",
            background:
              "radial-gradient(circle, rgba(139, 92, 246, 0.15) 0%, rgba(31, 41, 55, 0) 70%)",
            filter: "blur(40px)",
            zIndex: 0,
          }}
        />
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(transparent 95%, rgba(59, 130, 246, 0.1) 100%), linear-gradient(90deg, transparent 95%, rgba(59, 130, 246, 0.1) 100%)",
      ...
      <div className="max-w-4xl w-full mx-auto relative z-10">
        ...
        >
          <h1 className="text-7xl font-bold tracking-widest mb-4 relative">
            <span
              className="absolute text-blue-900 blur-sm"
              style={{ left: "2px", top: "2px" }}
            >
              PULSE REALM
            </span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-400 to-pink-600">
              PULSE REALM
            </span>
          </h1>
          <p className="text-lg text-blue-300 max-w-2xl mx-auto">
            Synchronize with the rhythm of the void, feel its pulse, and become
            one with the beat.
          </p>
        </motion.div>
          ...
          {tracks.map((track, index) => (
            ...
            >
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(135deg, rgba(255,255,255,0.3) 0%, rgba(255,255,255,0) 50%, rgba(0,0,0,0.2) 100%)",
                }}
              />
          ...
          <div className="flex gap-6 items-center justify-center">
            ...
            <div className="flex flex-col items-center text-center px-4">
              ...
              <p className="text-gray-300 max-w-lg">
                Watch for the circular waves approaching the{" "}
                <span className="text-green-400 font-bold">colored rings</span>.
                Press <span className="text-blue-400 font-bold">SPACE</span> or
                click when the waves align with the highlighted rings for
                perfect timing. Build combos for higher scores!
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="space-y-6 text-lg">
          <div className="flex items-center gap-4">
            ...
            <p className="text-white">
              Sound waves will approach the center from the outside.{" "}
              <span className="text-green-400 font-bold">Watch</span> for the
              colored rings!
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <p className="text-white">
              When a wave reaches the{" "}
              <span className="text-green-400 font-bold">green</span> or{" "}
              <span className="text-yellow-400 font-bold">yellow</span> rings,
              the rings will light up. That's your cue to press SPACE or click.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <p className="text-white">
              The green ring gives you{" "}
              <span className="text-green-400 font-bold">PERFECT</span> hits,
              while the yellow ring gives{" "}
              <span className="text-yellow-400 font-bold">GOOD</span> hits.
              Perfect timing = more points!
            </p>
          </div>

          <div className="bg-blue-900/30 p-4 rounded-lg border border-blue-500/30 mt-6">
            <p className="text-white text-center">Different wave types:</p>
            ...
            <motion.div
              ...
              style={{
                background: `rgb(${Math.floor(
                  Math.random() * 100 + 156
                )}, ${Math.floor(Math.random() * 100 + 156)}, 255)`,

...
export async function getWalletInfo(publicKey: PublicKey): Promise<WalletData> {
    try {
        ...
        const portfolioHistory = generateMockPortfolioHistory(mockPortfolioValue, transactions);
...
function generateMockPortfolioHistory(currentValue: number, transactions: any[]): Array<{date: string, value: number}> {
    ...
    try {
        ...
        try {
            ...
            if (nfts && nfts.length > 0) {
                ownedNFTs = await Promise.all(nfts.map(async (nft) => {
                    ...
                    const isMinted = 'updateAuthority' in nft && nft.updateAuthority?.toString() === publicKey.toString();
                    ...
                    const nftData: NFTData = {
                        ...
                        collection: metadata?.collection?.name || nft.collection?.address?.toString() || "VOID Collection",
                        description: metadata?.description || "A unique VOID NFT",
                        acquired: new Date().toISOString().split('T')[0],
                        minted: isMinted ? new Date().toISOString().split('T')[0] : undefined,
                        mintedAt: isMinted ? new Date().toISOString() : undefined,
                        price: randomValue,
                        type: metadata?.attributes?.find((attr: any) => attr.trait_type === "Type")?.value || "cube",
                        shapeType: metadata?.attributes?.find((attr: any) => attr.trait_type === "Shape")?.value || "complex",
                        color: metadata?.attributes?.find((attr: any) => attr.trait_type === "Color")?.value || "purple",
                        image: imageUrl,
                        attributes: metadata?.attributes || [],
                        mintAddress: nft.address.toString(),
                        ipfsHash: nft.uri?.includes('ipfs://') ? nft.uri.replace('ipfs://', '') : undefined,
        ...
        console.log(`Returning NFTs - Owned: ${ownedNFTs.length}, Minted: ${mintedNFTs.length}, Listed: ${listedNFTs.length}`);
...
function generateMockNFTValueHistory(currentValue: number): Array<{date: string, value: number}> {
    const history: Array<{date: string, value: number}> = [];
...
export async function getTransactionHistory(publicKey: PublicKey): Promise<TransactionData[]> {
    try {
        ...
        for (let i = 0; i < Math.min(10, signatures.length); i++) {
            ...
            if (tx) {
                ...
                if (tx.meta?.logMessages) {
                    if (tx.meta.logMessages.some(msg => msg.includes("Initialize mint"))) {
                        txType = "Mint";
                    } else if (tx.meta.logMessages.some(msg => msg.includes("Instruction: CreateAccount"))) {
                        txType = "Create Account";
                    } else if (tx.meta.logMessages.some(msg => msg.includes("Instruction: Transfer"))) {
                        txType = "Transfer";
                    } else if (tx.meta.logMessages.some(msg => msg.includes("Instruction: List"))) {
                        txType = "List NFT";
                    } else if (tx.meta.logMessages.some(msg => msg.includes("Instruction: Buy"))) {
                        txType = "Buy NFT";
                    }
                }
                
                // Calculate amount more accurately
                const amount = tx.meta?.postBalances && tx.meta?.preBalances ?
                    Math.abs(tx.meta.postBalances[0] - tx.meta.preBalances[0]) / LAMPORTS_PER_SOL : 0;
                
                // Format date
                const formattedDate = signature.blockTime ? 
                    new Date(signature.blockTime * 1000).toISOString().split('T')[0] : 'Unknown';
